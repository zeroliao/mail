...
type MailAppStore = {
  ...
  bindCredentials: (email: string, password: string) => Promise<{ message: string }>;
  bindOAuthAccount: (payload: BindOAuthPayload) => Promise<BindOAuthAccountResponse>;
  bindOAuthAccounts: (payload: BindOAuthPayload[]) => Promise<BindOAuthBatchResponse>;
...
const preserveSelection = async (
  ...
  const preserved = currentSelectedId ? nextItems.find((item) => item.id === currentSelectedId) : null;
...
export const useMailAppStore = create<MailAppStore>((set, get) => {
  const reloadAccounts = async () => {
    ...
    set({
      accounts,
      activeAccountId:
        activeAccountId === ALL_ACCOUNTS_ID || accounts.some((item) => item.id === activeAccountId)
          ? activeAccountId
          : ALL_ACCOUNTS_ID,
      composeDraft: {
        ...get().composeDraft,
  ...
  bootstrap: async () => {
    ...
    try {
      ...
      try {
        ...
        const folderCounts = await mailApi.getFolderCounts(ALL_ACCOUNTS_ID, accounts);
        set({
      ...
      set({
        ...
        backendError: error instanceof Error ? error.message : "后端服务不可用",
        authTokenReady: true,
        bootstrapped: true,
        isBootstrapping: false
      });
  ...
  refreshMailbox: async () => {
    const { activeAccountId, activeFolder, mailPagination, accounts, selectedMessage } = get();
    if (!accounts.length) {
      return;
    }
    ...
    await preserveSelection(nextPagination.items, selectedMessage?.id ?? null, get().openMessage);
  },

  selectAccount: async (accountId) => {
    const { activeFolder, mailPagination, accounts, composeDraft, selectedMessage } = get();
    set({
    ...
    set({
      ...
      composeDraft: {
        ...composeDraft,
        accountId:
          accountId === ALL_ACCOUNTS_ID ? composeDraft.accountId || accounts[0]?.id || "" : accountId
      }
    });
  ...
  selectFolder: async (folder) => {
    const { activeAccountId, mailPagination, accounts, selectedMessage } = get();
    set({
      activeFolder: folder,
      isLoadingMessages: true
    });
  ...
  changePage: async (page) => {
    ...
    const pageToken = page > mailPagination.page ? (mailPagination.nextPageToken ?? undefined) : undefined;
    const nextPagination = await mailApi.listMessages({
      accountId: activeAccountId,
      folder: activeFolder,
      page,
  ...
  openMessage: async (messageId, accountId) => {
    ...
    set({ isLoadingMessageDetail: true });
    const selectedMessage = (await mailApi.getMessage(targetAccountId, messageId, get().accounts)) ?? null;
  ...
  bindAccount: async (provider) => {
    ...
    try {
      const { authUrl } = await mailApi.bindAccount(provider, `${window.location.origin}/auth`);
  ...
  prepareReply: async (messageId) => {
    const message = get().selectedMessage?.id === messageId
      ? get().selectedMessage
  ...
  sendCompose: async () => {
    ...
    if (!composeDraft.accountId || !composeDraft.to.trim() || !composeDraft.subject.trim()) {
    ...
    try {
      const result = await mailApi.sendMail({
        ...
        to: composeDraft.to.split(",").map((item) => item.trim()).filter(Boolean),
        cc: composeDraft.cc.split(",").map((item) => item.trim()).filter(Boolean),
        bcc: composeDraft.bcc.split(",").map((item) => item.trim()).filter(Boolean),
