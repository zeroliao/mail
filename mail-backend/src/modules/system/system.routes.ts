...
export const systemRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/config/oauth-providers", {
    schema: {
      tags: ["system"]
    }
  }, async () => systemService.getOAuthProviderConfig());

  fastify.post("/accounts/oauth/init", {
  ...
  fastify.get("/auth/gmail/init", {
    ...
    const result = await accountsService.createOAuthUrl({ provider: MailProvider.GOOGLE });
    return {
      url: result.authUrl,
      authUrl: result.authUrl,
      state: result.state
  ...
  fastify.get("/auth/microsoft/init", {
    preHandler: [fastify.authenticate],
    schema: {
      tags: ["oauth"],
      security: [{ bearerAuth: [] }]
    ...
    const result = await accountsService.createOAuthUrl({ provider: MailProvider.MICROSOFT });
    return {
      url: result.authUrl,
      authUrl: result.authUrl,
      state: result.state
  ...
  fastify.get("/auth/gmail/callback", {
    schema: {
      tags: ["oauth"]
    }
  }, async (request) => {
  ...
  fastify.post("/mail/send", {
    ...
    return {
      message: result.providerMessageId ? "Mail sent successfully." : "Send request accepted by the backend.",
